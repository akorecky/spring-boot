...
public class InMemoryTodoRepositoryTest {
	...
	public void setup() {
		...
		todo = Todo.builder().title("My Todo").description("it's urgent").dueDate(LocalDate.now()).build();
	...
	public void shouldReplaceTodoWithSameId() {
		...
		Todo anotherTodo = Todo.builder().id(id).title("Another Todo").description("it's not as urgent")
				.dueDate(LocalDate.now().plus(1, ChronoUnit.DAYS)).build();
	...
	public void shouldHoldMultipleEntries() {
		...
		for (int i = 0; i < n; i++) {
			repository.save(
					Todo.builder().title("title" + i).description("description" + i).dueDate(LocalDate.now()).build());
		}
